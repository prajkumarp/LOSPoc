...
class SOTabBarItem: UIView {
    ...
    public lazy var titleLabel: UILabel = {
        let lbl = UILabel()
        lbl.text = self.title
        lbl.font = ThemeManager.sharedInstance.primaryTheme.fonts.headingMedium12
        lbl.textColor = ThemeManager.sharedInstance.primaryTheme.navBar_Text_Normal
    ...
    private func drawConstraints() {
        ...
        NSLayoutConstraint.activate([
            tabImageView.topAnchor.constraint(equalTo: self.topAnchor),
...
//            self?.titleLabel.frame.origin.y = SOTabBarSetting.tabBarHeight
            self?.tabImageView.frame.origin.y = 10
